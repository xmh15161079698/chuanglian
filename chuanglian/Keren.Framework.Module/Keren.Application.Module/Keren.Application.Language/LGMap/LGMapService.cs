...
namespace Keren.Application.Language
{
    ...
    public class LGMapService : RepositoryFactory
    {
        ...
        public IEnumerable<LanguageModel> GetDataList(string Code)
        {
            try
            {
                var sql = "SELECT F_Code as id,F_Name as name FROM KR_Lg_Map t WHERE F_TypeCode='" + Code + "' AND  t.F_Code <>'' AND t.F_Code<>'undefined'";
        ...
        public IEnumerable<LGMapEntity> GetListByTypeCode(string TypeCode)
        {
            try
            {
                var sql = "select * from KR_Lg_Map where  F_TypeCode='" + TypeCode + "' AND  t.F_Code <>'' AND t.F_Code<>'undefined'";
        ...
        public IEnumerable<LGMapEntity> GetList(string queryJson)
        {
            try
            {
                ...
                strSql.Append(" FROM KR_Lg_Map t WHERE 1=1 AND  t.F_Code <>'' AND t.F_Code<>'undefined'");
        ...
        public DataTable GetPageList(Pagination pagination, string queryJson, string typeList)
        {
            try
            {
                ...
                strSql.Append(" FROM KR_Lg_Map m ");
                for (var j = 1; j < list.Length; j++)
                {
                    strSql.Append("LEFT JOIN KR_Lg_Map m" + j + " ON m.F_Code=m" + j + ".F_Code AND m" + j + ".F_TypeCode='" + list[j] + "' ");
                }
        ...
        public IEnumerable<LGMapEntity> GetListByName(string keyValue)
        {
            try
            {
                ...
                strSql.Append("SELECT ");
                strSql.Append(fieldSql);
                strSql.Append(" FROM KR_Lg_Map t WHERE 1=1 AND  t.F_Code <>'' AND t.F_Code<>'undefined' AND t.F_Name='" + keyValue + "'");
                return this.BaseRepository().FindList<LGMapEntity>(strSql.ToString());
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<LGMapEntity> GetListByNameAndType(string keyValue, string typeCode)
        {
            try
            {
                ...
                strSql.Append("SELECT ");
                strSql.Append(fieldSql);
                strSql.Append(" FROM KR_Lg_Map t WHERE 1=1 AND  t.F_Code <>'' AND t.F_Code<>'undefined' AND t.F_Name='" + keyValue + "' AND t.F_TypeCode='" + typeCode + "'");
                return this.BaseRepository().FindList<LGMapEntity>(strSql.ToString());
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                {//编辑
                    ...
                    foreach (JProperty item in newproperties)
                    {
                        ...
                        {
                            var sql = "UPDATE KR_LG_MAP SET F_Name=@newName WHERE F_Name=@oldName AND F_Code=@code";
