    ...
    public class CodeGeneratorApp
    {
        ...
        public string FormCreate(BaseModel baseInfo, List<CodeFormTabModel> formData, Dictionary<string, CodeFormCompontModel> compontMap)
        {
            try
            {
                ...
                {
                    sb.Append("<div class=\"kr-form-wrap\">\r\n");
        ...
        public string ApiCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, Dictionary<string, CodeFormCompontModel> compontMap, ColModel colData)
        {
            try
            {
                ...
                sb.Append("namespace Keren.Application.WebApi\r\n");
                sb.Append("{\r\n");
                sb.Append(NotesCreate(baseInfo));
                ...
                sb.Append("            : base(\"/Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "\")\r\n");
                sb.Append("        {\r\n");
        ...
        public string AppIndexCreate(BaseModel baseInfo, Dictionary<string, CodeFormCompontModel> compontMap, QueryModel queryData)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("<div class=\"kr-page kr-" + baseInfo.outputArea + baseInfo.name + "-page\">\r\n");

                sb.Append("    <div class=\"kr-page-tool\">\r\n");
                sb.Append("        <div class=\"kr-tool-left\">\r\n");
                sb.Append("            总共<span class=\"kr-badge kr-badge-primary\">0</span>条\r\n");
                sb.Append("        </div>\r\n");
                sb.Append("        <div class=\"kr-tool-right\">\r\n");
                // 多条件查询
                if (queryData.fields.Count > 0)
                {
                    sb.Append("            <div class=\"kr-tool-right-btn kr_multiple_search\">\r\n");
                    sb.Append("                <i class=\"iconfont icon-searchlist\"></i>\r\n");
                    sb.Append("                <div class=\"kr-tool-right-btn-content kr-form-container\">\r\n");
                    foreach (var item in queryData.fields)
                    {
                        CodeFormCompontModel compont = compontMap[item.id];
                        sb.Append("                    <div class=\"kr-form-row\">\r\n");
                ...
                if (queryData.isDate == "1")
                {
                    sb.Append("            <div class=\"kr-tool-right-btn kr_time_search\" >\r\n");
                ...
                // 列表
                sb.Append("    <div class=\"kr-page-content\" id=\"kr_"+ baseInfo.outputArea + baseInfo.name + "_list\"></div>\r\n");

                // 添加按钮
                sb.Append("    <div class=\"kr-list-addbtn\" id=\"kr_" + baseInfo.outputArea + baseInfo.name + "_btn\">\r\n");
        ...
        public string AppIndexJSCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, Dictionary<string, CodeFormCompontModel> compontMap, ColModel colData, QueryModel queryData)
        {
            try
            {
                ...
                sb.Append("            page.grid = $page.find('#kr_" + baseInfo.outputArea + baseInfo.name + "_list').krpagination({\r\n");
                ...
                sb.Append("                    if ($et.hasClass('kr-btn-danger')) {\r\n");
                ...
                {
                    sb.Append("            // 时间搜索\r\n");
                    sb.Append("            $page.find('.kr_time_search').searchdate({\r\n");
                ...
                if (queryData.fields.Count > 0) {
                    sb.Append("            // 多条件查询\r\n");
                    sb.Append("            var $multiple = $page.find('.kr_multiple_search').multiplequery({\r\n");
                    ...
                    foreach (var item in queryData.fields)
                    {
                        ...
                        switch (compont.type) {
                            case "select":
                            case "radio":
                                sb.Append("            $multiple.find('#"+ compont.field + "').krpickerex({\r\n");
                                if (compont.dataSource == "0")
                                {
                                ...
                                break;
                            case "checkbox":
                                sb.Append("            $multiple.find('#" + compont.field + "').krcheckboxex({\r\n");
                                if (compont.dataSource == "0")
                                {
                                    sb.Append("                type: 'dataItem',\r\n");
                                    sb.Append("                code: '" + compont.itemCode + "'\r\n");
                            ...
                            case "currentInfo":
                                if (compont.dataType == "company" || compont.dataType == "department" || compont.dataType == "user")
                                {
                                    sb.Append("            $multiple.find('#" + compont.field + "').krselect({\r\n");
                ...
                // 新增按钮
                sb.Append("            $page.find('#kr_" + baseInfo.outputArea + baseInfo.name + "_btn').on('tap', function () {\r\n");
                sb.Append("                Keren.nav.go({ path: '"+ baseInfo.outputArea + "/" + baseInfo.name + "/form', title: '新增', type: 'right' });\r\n");
                sb.Append("            });\r\n");
                sb.Append("        },\r\n");
                sb.Append("        lclass: 'kr-list',\r\n");
                ...
                sb.Append("            }\r\n");
                sb.Append("            Keren.httpget(config.webapi + 'Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/pagelist', _postParam, (data) => {\r\n");
                sb.Append("                $page.find('.kr-badge').text('0');\r\n");
                sb.Append("                if (data) {\r\n");
                sb.Append("                    $page.find('.kr-badge').text(data.records);\r\n");
                ...
                sb.Append("            _$item.addClass('kr-list-item kr-list-item-multi');\r\n");
                foreach (var col in colData.fields)
                {
                    sb.Append("            _$item.append($('<p class=\"kr-ellipsis\"><span>" + col.fieldName + ":</span></p>').dataFormatter({ value: _item." + col.field);
                    CodeFormCompontModel compont = compontMap[col.id];
                ...
                sb.Append("            Keren.nav.go({ path: '" + baseInfo.outputArea + "/" + baseInfo.name + "/form', title: '详情', type: 'right', param: { keyValue: item." + mainPkey + " } });\r\n");
                sb.Append("        },\r\n");
                sb.Append("        btnClick: function (item, $item, $page) {// 左滑按钮点击事件\r\n");
                sb.Append("            Keren.layer.confirm('确定要删除该笔数据吗？', function (_index) {\r\n");
                sb.Append("                if (_index === '1') {\r\n");
                sb.Append("                    Keren.layer.loading(true, '正在删除该笔数据');\r\n");
                sb.Append("                    Keren.httppost(config.webapi + 'Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/delete', item."+ mainPkey + " , (data) => {\r\n");
                sb.Append("                        if (data) {// 删除数据成功\r\n");
                sb.Append("                            page.grid.reload();\r\n");
                sb.Append("                        }\r\n");
                sb.Append("                        Keren.layer.loading(false);\r\n");
                sb.Append("                    });\r\n");
                sb.Append("                }\r\n");
                sb.Append("            }, '系统提示', ['取消', '确定']);\r\n");
                sb.Append("        },\r\n");
                sb.Append("        rowBtns: ['<a class=\"kr-btn-danger\">删除</a>'] // 列表行左滑按钮\r\n");
                sb.Append("    };\r\n");
        ...
        private void AppCreateFormCompont(StringBuilder sb, List<CodeFormCompontModel> componts)
        {
            foreach (var compont in componts)
            {
                if (compont.type != "label"){
                    ...
                    {
                        sb.Append("    <div class=\"kr-form-row\" id=\"" + compont.table + "\" ></div>\r\n");
                    }
                    else {
                        string m = "";
                        ...
                        sb.Append("    <div class=\"kr-form-row  "+ m + "\" data-table=\"" + compont.table + "\">\r\n");
        ...
        public string AppFormCreate(BaseModel baseInfo, List<CodeFormTabModel> formData, Dictionary<string, CodeFormCompontModel> compontMap)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("<div class=\"kr-form-container\">\r\n");
                foreach (var tab in formData)
                {
                    if (formData.Count > 1) {
                        sb.Append("    <div class=\"kr-form-row kr-form-row-title\">\r\n");
        ...
        public string AppFormJsCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, List<CodeFormTabModel> formData, Dictionary<string, CodeFormCompontModel> compontMap)
        {
            try
            {
                ...
                sb.Append("            // 添加头部按钮列表\r\n");
                sb.Append("            var _html = '\\\r\n");
                sb.Append("                <div class=\"kr-form-header-cancel\" >取消</div>\\\r\n");
                sb.Append("                <div class=\"kr-form-header-btnlist\" >\\\r\n");
                sb.Append("                    <div class=\"kr-form-header-more\" ><i class=\"iconfont icon-more\" ></i></div>\\\r\n");
                sb.Append("                    <div class=\"kr-form-header-edit\" ><i class=\"iconfont icon-edit\" ></i></div>\\\r\n");
                sb.Append("                </div>\\\r\n");
                sb.Append("                <div class=\"kr-form-header-submit\" >提交</div>';\r\n");
                sb.Append("            $header = $page.parents('.f-page').find('.f-page-header');\r\n");
                sb.Append("            $header.append(_html);\r\n");
                sb.Append("            // 取消\r\n");
                sb.Append("            $header.find('.kr-form-header-cancel').on('tap', function () {\r\n");
                sb.Append("                Keren.layer.confirm('确定要退出当前编辑？', function (_index) {\r\n");
                sb.Append("                    if (_index === '1') {\r\n");
                sb.Append("                        if (keyValue) {// 如果是编辑状态\r\n");
                sb.Append("                            Keren.formblur();\r\n");
                sb.Append("                            $header.find('.kr-form-header-cancel').hide();\r\n");
                sb.Append("                            $header.find('.kr-form-header-submit').hide();\r\n");
                sb.Append("                            $header.find('.kr-form-header-btnlist').show();\r\n");
                sb.Append("                            $header.find('.f-page-title').text(titleText);\r\n");
                sb.Append("                            $page.find('.kr-form-container').setFormRead();\r\n");
                sb.Append("                        }\r\n");
                sb.Append("                        else {// 如果是新增状态 关闭当前页面\r\n");
                sb.Append("                            Keren.nav.closeCurrent();\r\n");
                sb.Append("                        }\r\n");
                sb.Append("                    }\r\n");
                sb.Append("                }, '系统提示', ['取消', '确定']);\r\n");
                sb.Append("            });\r\n");
                sb.Append("            // 编辑\r\n");
                sb.Append("            $header.find('.kr-form-header-edit').on('tap', function () {\r\n");
                sb.Append("                $header.find('.kr-form-header-btnlist').hide();\r\n");
                sb.Append("                $header.find('.kr-form-header-cancel').show();\r\n");
                sb.Append("                $header.find('.kr-form-header-submit').show();\r\n");
                sb.Append("                titleText = $header.find('.f-page-title').text();\r\n");
                sb.Append("                $header.find('.f-page-title').text('编辑');\r\n");
                sb.Append("                $page.find('.kr-form-container').setFormWrite();\r\n");
                sb.Append("            });\r\n");
                sb.Append("            // 更多\r\n");
                sb.Append("            $header.find('.kr-form-header-more').on('tap', function () {\r\n");
                sb.Append("                Keren.actionsheet({\r\n");
                sb.Append("                    id: 'more',\r\n");
                sb.Append("                    data: [\r\n");
                sb.Append("                        {\r\n");
                sb.Append("                            text: '删除',\r\n");
                ...
                sb.Append("                                    if (_index === '1') {\r\n");
                sb.Append("                                        Keren.layer.loading(true, '正在删除该笔数据');\r\n");
                sb.Append("                                        Keren.httppost(config.webapi + 'Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/delete', keyValue, (data) => {\r\n");
                sb.Append("                                            Keren.layer.loading(false);\r\n");
                sb.Append("                                            if (data) {// 删除数据成功\r\n");
                sb.Append("                                                Keren.nav.closeCurrent();\r\n");
                sb.Append("                                                var prepage = Keren.nav.getpage('"+ baseInfo.outputArea + "/" + baseInfo.name + "');\r\n");
                ...
                sb.Append("            });\r\n");
                sb.Append("            // 提交\r\n");
                sb.Append("            $header.find('.kr-form-header-submit').on('tap', function () {\r\n");
                sb.Append("                // 获取表单数据\r\n");
                sb.Append("                if (!$page.find('.kr-form-container').krformValid()) {\r\n");
                sb.Append("                    return false;\r\n");
                sb.Append("                }\r\n");
                ...
                if (dbTableList.Count == 1)
                {
                    sb.Append("                _postData.strEntity = JSON.stringify($page.find('.kr-form-container').krformGet());\r\n");
                }
                else {
                    foreach (var table in dbTableList)
                    {
                        if (girdcomponts.FindAll(t => t.table == table.name).Count >= 1)
                        {
                            sb.Append("                _postData.str" + Str.FirstLower(table.name) + "List = JSON.stringify($page.find('#" + table.name + "').krgridGet());\r\n");
                        }
                        else
                        {
                            if (string.IsNullOrEmpty(table.relationName))
                            {
                                sb.Append("                _postData.strEntity = JSON.stringify($page.find('[data-table=\"" + table.name + "\"]').krformGet());\r\n");
                            }
                            else
                            {
                                sb.Append("                _postData.str" + Str.FirstLower(table.name) + "Entity = JSON.stringify($page.find('[data-table=\"" + table.name + "\"]').krformGet());\r\n");
                            }
                        }
                    }
                }
                sb.Append("                Keren.layer.loading(true, '正在提交数据');\r\n");
                sb.Append("                Keren.httppost(config.webapi + 'Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/save', _postData, (data) => {\r\n");
                sb.Append("                    Keren.layer.loading(false);\r\n");
                sb.Append("                    if (data) {// 表单数据保存成功\r\n");
                sb.Append("                        if (keyValue) {\r\n");
                sb.Append("                            Keren.layer.toast('保存数据成功!');\r\n");
                sb.Append("                            Keren.formblur();\r\n");
                sb.Append("                            $header.find('.kr-form-header-cancel').hide();\r\n");
                sb.Append("                            $header.find('.kr-form-header-submit').hide();\r\n");
                sb.Append("                            $header.find('.kr-form-header-btnlist').show();\r\n");
                ...
                sb.Append("                $header.find('.kr-form-header-btnlist').show();\r\n");
                sb.Append("                // 获取表单数据\r\n");
                sb.Append("                Keren.layer.loading(true, '获取表单数据');\r\n");
                sb.Append("                Keren.httpget(config.webapi + 'Keren/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/form', keyValue, (data) => {\r\n");
                sb.Append("                    if (data) {\r\n");
                sb.Append("                        for (var id in data) {\r\n");
                sb.Append("                            if (data[id].length) {\r\n");
                sb.Append("                                $page.find('#' + id ).krgridSet(data[id]);\r\n");
                sb.Append("                            }\r\n");
                sb.Append("                            else {\r\n");
                sb.Append("                                $page.find('[data-table=\"' + id + '\"]').krformSet(data[id]);\r\n");
                sb.Append("                            }\r\n");
                sb.Append("                        }\r\n");
                sb.Append("                    }\r\n");
                sb.Append("                    Keren.layer.loading(false);\r\n");
                ...
                foreach (var tab in formData)
                {
                    foreach (var compont in tab.componts)
                    {
                        ...
                        {
                            case "girdtable":
                                sb.Append("            $page.find('#" + compont.table + "').krgrid({\r\n");
                                ...
                                break;
                            case "checkbox":
                                sb.Append("            $page.find('#"+ compont.field + "').krcheckboxex({\r\n");
                                if (compont.dataSource == "0")
                                {
                                    sb.Append("            code: '" + compont.itemCode + "',\r\n");
                                    sb.Append("            type: 'dataItem'");
                            ...
                            case "radio":
                            case "select":
                                sb.Append("            $page.find('#" + compont.field + "').krpickerex({\r\n");
                                if (compont.dataSource == "0")
                                {
                                    sb.Append("            code: '" + compont.itemCode + "',\r\n");
                                    sb.Append("            type: 'dataItem'");
                                ...
                                break;
                            case "datetime":
                                sb.Append("            $page.find('#" + compont.field + "').krdate({\r\n");
                            ...
                            case "datetimerange":
                                ...
                                sb.Append("                if (st && et) {\r\n");
                                sb.Append("                    var diff = Keren.date.parse(st).DateDiff('d', et) + 1;\r\n");
                            ...
                            case "encode":
                                sb.Append("            if (!keyValue) {\r\n");
                                sb.Append("                Keren.getRuleCode('"+ compont.rulecode + "', function (data) {\r\n");
                                sb.Append("                    $page.find('#"+ compont.field + "').val(data);\r\n");
                                sb.Append("                });\r\n");
                                sb.Append("            }\r\n");
                                break;
                            case "organize":
                                sb.Append("            $page.find('#" + compont.field + "').krselect({\r\n");
                                sb.Append("                type: '"+ compont.dataType + "',\r\n");
                                ...
                                if (!string.IsNullOrEmpty(compont.relation))
                                {
                                    ...
                                    sb.Append("                var value = $(this).krselectGet();\r\n");
                                    sb.Append("                $page.find('#" + compont.field + "').krselectUpdate({\r\n");
                            ...
                            case "currentInfo":
                                ...
                                {
                                    sb.Append("            if (!keyValue) {\r\n");
                                    sb.Append("            $page.find('#" + compont.field + "').val(Keren.date.format(new Date(), 'yyyy-MM-dd hh:mm:ss'));\r\n");
                                    sb.Append("            }\r\n");
                                }
                                else {
                                    sb.Append("            $page.find('#" + compont.field + "').krselect({\r\n");
                                    ...
                                    {
                                        case "company":
                                            sb.Append("            $page.find('#" + compont.field + "').krselectSet(Keren.storage.get('userinfo').baseinfo.companyId);\r\n");
                                            break;
                                        case "department":
                                            sb.Append("            $page.find('#" + compont.field + "').krselectSet(Keren.storage.get('userinfo').baseinfo.departmentId);\r\n");
                                            break;
                                        case "user":
                                            sb.Append("            $page.find('#" + compont.field + "').krselectSet(Keren.storage.get('userinfo').baseinfo.userId);\r\n");
                                            break;
                                    }
                                    sb.Append("            }\r\n");
                                }
