...
namespace Keren.Application.Base.SystemModule
{
    ...
    public class AnnexesFileBLL : AnnexesFileIBLL
    {
        ...
        private string cacheKey = "Keren_ADMS_annexes_";
        #region 获取数据
        /// <summary>
        /// 获取实体列表
        /// </summary>
        ...
        /// <returns></returns>
        public bool SaveQiNiuAnnexes(string folderId, string fileGuid, string fileName, int chunks, UserInfo userInfo)
        {
            try
            {
                //文件处理
                string FileEextension = Path.GetExtension(fileName);
                string fullFileName = fileGuid+ FileEextension;
                string qiniuHost = Util.Config.GetValue("QiNiuHost");
                string AK = Util.Config.GetValue("AccessKey");
                string SK = Util.Config.GetValue("SecretKey");
                bool httpsMark = Util.Config.GetValue("HttpsMark")=="true"?true:false;
                string bucket = Util.Config.GetValue("QiNiuBucket");
                string saveKey = fullFileName;
                // 上传策略，参见 
                ...
                Qiniu.Common.Config.AutoZone(AK,bucket, httpsMark);
                ...
                for (int i = 0; i < chunks; i++)
                {
                    ...
                    //写入二进制流
                    HttpResult result = fu.UploadData(bufferByRedis, saveKey, token);
                ...
                fileAnnexesEntity.F_FilePath = qiniuHost+fullFileName;
        ...
        {
            string storageType = Util.Config.GetValue("StorageType");
            switch (storageType)
            {
                case "local":return SaveLocalAnnexes(folderId, fileGuid, fileName, chunks, userInfo, filePath);
                case "qiniu":return SaveQiNiuAnnexes(folderId, fileGuid, fileName, chunks,userInfo);
        ...
        /// <param name="filePath"></param>
        /// <returns></returns>
        public bool SaveLocalAnnexes(string folderId, string fileGuid, string fileName, int chunks, UserInfo userInfo, string filePath = "")
        {
            try
            {
                //获取文件完整文件名(包含绝对路径)
                ...
                {
                    filePath = Util.Config.GetValue("AnnexesFile");
        ...
        public string SaveAnnexes(string fileGuid, string fileName, int chunks, UserInfo userInfo)
        {
            try
            {
                //获取文件完整文件名(包含绝对路径)
                //文件存放路径格式：/Resource/ResourceFile/{userId}/{date}/{guid}.{后缀名}
                string filePath = Util.Config.GetValue("AnnexesFile");
                string uploadDate = DateTime.Now.ToString("yyyyMMdd");
                string FileEextension = Path.GetExtension(fileName);
