    ...
    public class TSProcessService : RepositoryFactory 
    {
        ...
        public IEnumerable<TSProcessEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                FROM
	                KR_TS_Process t
                LEFT JOIN 
	                KR_TS_SchemeInfo s ON s.F_Id = t.F_SchemeInfoId
                ");
        ...
        public void SaveEntity(string keyValue, TSProcessEntity entity)
        {
            ...
            try
            {
                ...
                {
                    var entityTmp = db.FindEntity<TSProcessEntity>("SELECT * FROM KR_TS_Process Where F_SchemeInfoId = @schemeId AND  F_State != 10 ", new { schemeId = entity.F_SchemeInfoId });
                    if (entityTmp == null) {
                        db.ExecuteBySql("delete FROM KR_TS_Process Where F_SchemeInfoId = @schemeId AND  F_State != 10 ", new { schemeId = entity.F_SchemeInfoId });
                    }
