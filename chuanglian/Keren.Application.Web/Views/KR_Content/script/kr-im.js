(function ($, keren) {
            ...
            if (imChat) {
                ...
                imChat.client.revMsg = function (userId, msg, dateTime, isSystem) {
                    if (!loadingMsg2[userId]) {
                        ...
                        keren.im.revMsg && keren.im.revMsg(userId, msg, dateTime, isSystem || 0);
                    }
                }
            }
        }
    ...
    keren.im = {
        addContacts: function (userId) {// 添加联系人
            keren.httpAsync('Post', top.$.rootUrl + '/KR_IM/IMMsg/AddContact', { otherUserId: userId }, function (data) {});
        },
        removeContacts: function (userId) {// 移除联系人
            keren.httpAsync('Post', top.$.rootUrl + '/KR_IM/IMMsg/RemoveContact', { otherUserId: userId }, function (data) { });
        },
        getContacts: function (callback) {// 获取最近的联系人列表
            setTimeout(function () {
                keren.httpAsync('GET', top.$.rootUrl + '/KR_IM/IMMsg/GetContactsList', {}, function (res) {
                    if (res) {
                        _im.init();
        ...
        updateContacts: function (userId) {
            keren.httpAsync('Post', top.$.rootUrl + '/KR_IM/IMMsg/UpdateContactState', { otherUserId: userId}, function (data) {
            });
        },
        sendMsg: function (userId, msg) {// 发送消息
            var time = "";
            var loginInfo = keren.clientdata.get(['userinfo']);
            var point = { userId: loginInfo.userId, content: msg, time: keren.getDate('yyyy-MM-dd hh:mm:ss'), isSystem: 0 };
            addMsgList(userId, point);
            keren.httpAsync('Post', top.$.rootUrl + '/KR_IM/IMMsg/SendMsg', { userId: userId, content: msg }, function (data) {
                _im.sendMsg(userId, msg);// 发送给即时通讯服务
            });
            if (msgList[userId].length > 1) {
                if (keren.parseDate(point.time).DateDiff('s', msgList[userId][msgList[userId].length - 2].time) > 60) {
                    time = point.time;
                }
            }
            else {
        ...
        getMsgList: function (userId, callback,isGetMsgList) {
            ...
            if (msgList[userId].length == 0 && isGetMsgList) {// 如果没有信息，获取最近10条的聊天记录
                ...
                keren.httpAsync('GET', top.$.rootUrl + '/KR_IM/IMMsg/GetMsgList', { userId: userId }, function (data) {
                    msgList[userId] = msgList[userId] || [];
                    data = data || [];
                    var len = data;
                    if (len > 0) {
