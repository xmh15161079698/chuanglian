...
top.keren = (function ($) {
    ...
    var keren = {
        ...
        loading: function (isShow, _text) {//加载动画显示与否
            var $loading = top.$("#kr_loading_bar");
            if (!!_text) {
                top.keren.language.get(_text, function (text) {
                    top.$("#kr_loading_bar_message").html(text);
                });

            } else {
                top.keren.language.get("正在拼了命为您加载…", function (text) {
                    top.$("#kr_loading_bar_message").html(text);
                });
            }
            if (isShow) {
        ...
        countFileSize: function (size) {
            if (size < 1024.00)
                return keren.toDecimal(size) + " 字节";
            else if (size >= 1024.00 && size < 1048576)
                return keren.toDecimal(size / 1024.00) + " KB";
            else if (size >= 1048576 && size < 1073741824)
                return keren.toDecimal(size / 1024.00 / 1024.00) + " MB";
            else if (size >= 1073741824)
                return keren.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
        },
        // 数组复制
        ...
        checkrow: function (id) {
            ...
            if (id == undefined || id == "" || id == 'null' || id == 'undefined') {
                isOK = false;
                top.keren.language.get('您没有选中任何数据项,请选中后再操作！', function (text) {
                    keren.alert.warning(text);
                });
        ...
        alert: {
            success: function (msg) {
                top.keren.language.get(msg, function (text) {
                    toastr.success(text);
                });
