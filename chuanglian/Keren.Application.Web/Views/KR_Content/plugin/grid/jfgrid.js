(function ($, keren) {
    ...
    var setLastColWidth = function ($self, op, isUpdate) {
        ...
        if (op.running.cols.length > 0) {
            ...
            if (flag) {
                ...
                if (isUpdate) {
                    $self.find('#jfgrid_body_' + op.id + '>.kr-scroll-box').css({ 'width': (op.running.headWidth + op.running.leftWidth + _width - 1) });
    ...
    var formatterCell = function (value, item, row, rowItem, op) {
        ...
        if (item.formatter) {
            ...
            if (item.statistics) {// 如果该单元格数据需要统计就进行计算
                ...
                $('#jfgrid_statistic_' + op.id + ' [name="' + item.name + '"]').text(keren.toDecimal(op.running.statisticData[item.name]));
            }
                ...
                switch (item.edit.type) {
                    ...
                    case 'datatime':       // 时间
                        if (item.edit.dateformat == "0") {
                            value = keren.formatDate(value, "yyyy-MM-dd");
                            row[item.name] = value;
                        }
                    ...
                    case 'layer':          // 弹层
                        ...
                        if (item.isRead != '1') {
                            rowItem.$cell.find('.fa-ellipsis-h').on('click', function () {
                                ...
                                top.krGirdLayerEdit = op;
                                top.krGirdLayerEditCallBack = function (data) {
                                    op.edit.change && op.edit.change(_row, rownum, data);
                                    top.krGirdLayerEdit = null;
                                    top.krGirdLayerEditCallBack = null;
                                };
                                if (item.edit.op) {
                                    keren.layerForm({
                                        id: 'lrgridlayerform',
    ...
    var initEditCell = function ($self, op, col) {
        switch (col.data.edit.type) {
            ...
            case 'select':         // 下拉框选择
                ...
                col.$edit.find('div').krselect(col.data.edit.op);
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    keren.clientdata.getAllAsync('dataItem', {
                        ...
                        callback: function (dataes) {
                            ...
                            col.$edit.find('div').krselectRefresh({ data: list });
                        }
                    });
                }
                    ...
                    keren.clientdata.getAllAsync('sourceData', {
                        code: col.data.edit.code,
                        callback: function (dataes) {
                            col.$edit.find('div').krselectRefresh({
                                data: dataes
                            });
                        }
                    });
            ...
            case 'radio':          // 单选
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    col.$edit.find('div').krRadioCheckbox({
                        type: 'radio',
                        code: col.data.edit.code
                    });
                }
    ...
    var hideEditCell = function () {
        ...
        $('.jfgrid-layout .kr-select-option').slideUp(150);
        $('.jfgrid-layout .kr-select').removeClass('kr-select-focus');
    };
    ...
    var _jfgrid = {
                    ...
                    if (op.running.moveCol) {
                        ...
                        if (_width != 0){
                            ...
                            if (op.running.moveCol.frozen) {
                                ...
                                e.data.$obj.find('#jfgrid_body_' + op.id + '>.kr-scroll-box').css({ 'padding-left': op.running.leftWidth });
        ...
        head: function ($self, op) {
            ...
            $self.find('#jfgrid_body_' + op.id +'>.kr-scroll-box').css({ 'padding-left': op.running.leftWidth });
            ...
            $.each(op.running.headData, function (_index, _item) {
                ...
                // 翻译
                keren.language.get((_item.data.label || ""), function (text) {
        ...
        reload: function ($self, op) {
            ...
            if (op.isPage) {
                ...
                keren.httpAsync('GET', op.url, op.param, function (data) {
                    keren.loading(false);
                    if (data) {
        ...
        clickRow: function ($self, $et, op) {
            ...
            $.each(op.running.headData, function (i, col) {
                if (col.data.name == colname) {
                    if (col.data.edit && col.$edit && col.data.edit.type != 'layer' && col.data.isRead != '1' && rowData.krnotedit != '1' ) {
                        isEdit = true;
                        ...
                        switch (col.data.edit.type){
                            ...
                            case 'select':         // 下拉框选择
                                if (rowData[colname] == 0) {
                                    $edit.krselectSet(rowData[colname]);
                                }
                                else {
                                    $edit.krselectSet(rowData[colname] || '');
                                }
        ...
        expandSub: function ($self, $et, op) {
            ...
            if (rowItem['jfsubGrid'].value) {
                ...
                var subContentId = keren.newGuid();
