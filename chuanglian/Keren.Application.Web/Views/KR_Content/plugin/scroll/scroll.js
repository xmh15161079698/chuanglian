...
(function ($, keren, window) {
    ...
    var methods = {
        init: function ($this, callback) {
            var id = $this.attr('id');
            if (!id) {
                id = 'kr_' + keren.newGuid();
                $this.attr('id', id);
            }

            $this.addClass('kr-scroll-wrap');
            // 加载内容
            var $content = $this.children();

            var $scroll = $('<div class="kr-scroll-box" id="' + id + '_box" ></div>');
            $this.append($scroll);
            $scroll.append($content);

            // 加载y滚动条
            var $vertical = $('<div class="kr-scroll-vertical"   ><div class="kr-scroll-vertical-block" id="' + id + '_vertical"></div></div>')
            $this.append($vertical);

            // 加载x滚动条
            var $horizontal = $('<div class="kr-scroll-horizontal" ><div class="kr-scroll-horizontal-block" id="' + id + '_horizontal"></div></div>')
        ...
        bindEvent: function ($this, $scroll, $vertical, $horizontal) { // 绑定监听事件
            ...
            $this.mousewheel(function (event, delta, deltaX, deltaY) {
                ...
                var $this = $(this);
                var $select = $this.find('.kr-select-focus');
                if ($select.length > 0) {
                    var selectId = "keren_select_option_" + $select.attr('id');
            ...
            // 监听鼠标移动
            $vertical.find('.kr-scroll-vertical-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
                op.isYMousedown = true;
                op.yMousedown = e.pageY;
                $this.addClass('kr-scroll-active');
                $this = null;
            });
            $horizontal.find('.kr-scroll-horizontal-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
            ...
            top.$(document).on('mousemove', { $obj: $this }, function (e) {
                ...
                if (op.isYMousedown) {
                    ...
                    var $select = e.data.$obj.find('.kr-select-focus');
                    if ($select.length > 0) {
                        var selectId = "keren_select_option_" + $select.attr('id');
                        $('#' + selectId).slideUp(150);
                        $select.removeClass('kr-select-focus');
                ...
                $move.hide();
                e.data.$obj.removeClass('kr-scroll-active');
            });
        },
    ...
    $.fn.krscrollSet = function (name, data, isCallBack) {
        var $this = $(this);
        if (!$this[0] || !$this[0].op) {
            return;
        }
...
})(window.jQuery, top.keren, window);
