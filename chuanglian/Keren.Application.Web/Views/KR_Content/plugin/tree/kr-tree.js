...
(function ($, keren) {
    ...
    $.krtree = {
        ...
        render: function ($self) {
            ...
            // 渲染成树
            var $treeRoot = $('<ul class="kr-tree-root" ></ul>');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.krtree.renderNode(dfop.data[i], 0, i, dfop);
        ...
        renderNode: function (node, deep, path, dfop) {
            ...
            var $node = $('<li class="kr-tree-node"></li>');
            var $nodeDiv = $('<div id="' + dfop.id + '_' + nid + '" tpath="' + path + '" title="' + title + '"  dataId="' + dfop.id + '"  class="kr-tree-node-el" ></div>');
            if (node.hasChildren) {
                var c = (node.isexpand || dfop.isAllExpand) ? 'kr-tree-node-expanded' : 'kr-tree-node-collapsed';
                $nodeDiv.addClass(c);
            }
            else {
                $nodeDiv.addClass('kr-tree-node-leaf');
            }
            // span indent
            var $span = $('<span class="kr-tree-node-indent"></span>');
            if (deep == 1) {
                $span.append('<img class="kr-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            }
            else if (deep > 1) {
                $span.append('<img class="kr-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            ...
            $nodeDiv.append($span);
            // img
            var $img = $('<img class="kr-tree-ec-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            $nodeDiv.append($img);
            // checkbox
            if (node.showcheck) {
                ...
                var $checkBox = $('<img  id="' + dfop.id + '_' + nid + '_cb" + class="kr-tree-node-cb" src="' + dfop.cbiconpath + dfop.icons[node.checkstate] + '" />');
            ...
            // a
            var ahtml = '<a class="kr-tree-node-anchor" href="javascript:void(0);">';
            ahtml += '<span data-value="' + node.id + '" class="kr-tree-node-text" >' + node.text + '</span>';
            ahtml += '</a>';
            $nodeDiv.append(ahtml);
            // 节点事件绑定
            $nodeDiv.on('click', $.krtree.nodeClick);

            if (!node.complete) {
                $nodeDiv.append('<div class="kr-tree-loading"><img class="kr-tree-ec-icon" src="' + dfop.cbiconpath + 'loading.gif"/></div>');
            }

            $node.append($nodeDiv);
            if (node.hasChildren) {
                var $treeChildren = $('<ul  class="kr-tree-node-ct" >');
                ...
                if (node.ChildNodes) {
                    ...
                    for (var k = 0; k < l; k++) {
                        ...
                        var $childNode = $.krtree.renderNode(node.ChildNodes[k], deep + 1, path + "." + k, dfop);
        ...
        renderNodeAsync: function ($this, node, dfop) {
            ...
            if (node.ChildNodes) {
                ...
                for (var k = 0; k < l; k++) {
                    node.ChildNodes[k].parent = node;
                    var $childNode = $.krtree.renderNode(node.ChildNodes[k], node._deep + 1, node._path + "." + k, dfop);
                    $treeChildren.append($childNode);
                }
        ...
        renderToo: function ($self) {
            var dfop = $self[0]._lrtree.dfop;
            // 渲染成树
            var $treeRoot = $self.find('.kr-tree-root');
            $treeRoot.html('');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.krtree.renderNode(dfop.data[i], 0, i, dfop);
        ...
        nodeClick: function (e) {
            ...
            if (et.tagName == 'IMG') {
                var $et = $(et);
                var $ul = $this.next('.kr-tree-node-ct');
                if ($et.hasClass("kr-tree-ec-icon")) {
                    if ($this.hasClass('kr-tree-node-expanded')) {
                        $ul.slideUp(200, function () {
                            $this.removeClass('kr-tree-node-expanded');
                            $this.addClass('kr-tree-node-collapsed');
                        });
                    }
                    else if ($this.hasClass('kr-tree-node-collapsed')) {
                        // 展开
                        var path = $this.attr('tpath');
                        var node = $.krtree.getItem(path, dfop);
                        if (!node.complete) {
                            if (!node._loading) {
                                node._loading = true;// 表示正在加载数据
                                $this.find('.kr-tree-loading').show();
                                keren.httpAsync('GET', dfop.url, { parentId: node.id }, function (data) {
                                    if (!!data) {
                                        node.ChildNodes = data;
                                        $ul = $.krtree.renderNodeAsync($this, node, dfop);
                                        $ul.slideDown(200, function () {
                                            $this.removeClass('kr-tree-node-collapsed');
                                            $this.addClass('kr-tree-node-expanded');
                                        });
                                        node.complete = true;
                                        $this.find('.kr-tree-loading').hide();
                                    }
                                    node._loading = false;
                                });
                            }
                ...
                else if ($et.hasClass("kr-tree-node-cb")) {
                    var path = $this.attr('tpath');
                    var node = $.krtree.getItem(path, dfop);
                    ...
                    $.krtree.checkChild($.krtree.check, node, node.checkstate, dfop);
                    $.krtree.checkParent($.krtree.check, node, node.checkstate, dfop);
                    if (!!dfop.nodeCheck) {
                ...
                $('#' + dfop.id).find('.kr-tree-selected').removeClass('kr-tree-selected');
                $this.addClass('kr-tree-selected');
                if (!!dfop.nodeClick) {
                    dfop.nodeClick(node, $this);
                }
                if (dfop.isTextCheck) {
                    setTimeout(function () {
                        $this.find('.kr-tree-node-cb').trigger('click');
                    });
                }
            }
            return false;
            ...
            if (fn(node, state, 1, dfop) != false) {
                if (node.ChildNodes != null && node.ChildNodes.length > 0) {
                    ...
                    for (var i = 0, len = cs.length; i < len; i++) {
                        $.krtree.checkChild(fn, cs[i], state, dfop);
                    }
                }
            }
        },
        ...
        search: function (keyword, data) {
            ...
            $.each(data, function (i, row) {
                ...
                if (!keren.validator.isNotNull(keyword).code || row.text.indexOf(keyword) != -1) {
                    
                    flag = true;
                }
                if (row.hasChildren) {
                    if ($.krtree.search(keyword, row.ChildNodes)) {
                        flag = true;
                    }
                }
                if (flag) {
        ...
        addNode: function ($self, node, Id, index) {// 下一版本完善
            ...
            if (!!Id)// 在最顶层
            {
                ...
                var $node = $.krtree.renderNode(node, 0, index, dfop);
                if ($self.find('.kr-tree-root>li').length == 0) {
                    $self.find('.kr-tree-root>li').append($node);
                }
                else {
                    $self.find('.kr-tree-root>li').eq(index).before($node);
                }
                ...
                var $node = $.krtree.renderNode(node, 0, path + '.' + index, dfop);
    ...
    $.fn.krtree = function (settings) {
        var dfop = {
            ...
            data: [],
            isAllExpand: false,
            cbiconpath: top.$.rootUrl + '/Content/images/kerentree/',
            // 点击事件（节点信息）,节点$对象
            nodeClick: false,
            // 选中事件（节点信息）,节点$对象
            nodeCheck: false
        ...
        if (dfop.id == null || dfop.id == "") {
            dfop.id = "kerentree" + new Date().getTime();
            $self.attr("id", dfop.id);
        }
        $self.html('');
        $self.addClass("kr-tree");
    ...
    $.fn.krtreeSet = function (name,op) {
        ...
        switch (name) {
            case 'allNoCheck':
                $self.find('.kr-tree-node-cb').attr('src', dfop.cbiconpath + 'checkbox_0.png');
                setNoCheck(dfop.data);
                break;
            case 'allCheck':
                $self.find('.kr-tree-node-cb[src$="checkbox_0.png"]').each(function () {
                    var $this = $(this);
                    if ($this.parent().parent().find('.kr-tree-node-ct').length == 0) {
                        $this.trigger('click');
                    }
                    $this = null;
                });
            ...
            case 'setCheck':
                ...
                $.each(list, function (id, item) {
                    ...
                    if ($div.next().length == 0) {
                        $div.find('.kr-tree-node-cb').trigger('click');
                    }
                });
                break;
            case 'setValue':
            ...
            case 'getCheckNodeIdsByPath':
                ...
                break;
            case 'search':
                $.krtree.search(op.keyword, dfop.data);
                if (keren.validator.isNotNull(op.keyword).code) {
                    dfop._isSearch = true;
                }
                else if (dfop._isSearch) {
                    dfop._isSearch = false;
                }
                $.krtree.renderToo($self);
                break;
            case 'refresh':
                $.extend(dfop, op || {});
                if (!!dfop.url) {
