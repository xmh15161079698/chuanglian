$(function () {
    ...
    function target(data) {
        if (data.length > 0) {// 计算每块统计区域宽度
            ...
            $.each(data, function (_index, _item) {
                ...
                var _html = '\
                    <div class="kr-item-20" data-Id="'+ _item.F_Id +'">\
                        <div class="task-stat">\
                            <div class="visual">\
            ...
            $target.find('.kr-item-20').on('click', function () {
    ...
    function list(data) {
        if (data.length > 0) {
            ...
            var $list = $('#kr_desktop_list .kr-scroll-box');

            $.each(data, function (_index, _item) {
                listMap[_item.F_Id] = _item;
                var _html = '\
                <div class="kr-desktop-list"  data-Id="'+ _item.F_Id + '">\
                    <div class="title">\
                        '+ _item.F_Name +'\
                ...
                top.keren.httpAsync('GET', top.$.rootUrl + '/KR_Desktop/DTList/GetSqlData', { Id: _item.F_Id }, function (data) {
                    if (data) {
                        ...
                        $content.find('.kr-list-line').on('click', function () {
                            var $p = $(this).parent();
                            var id = $p.attr('data-value');
                            var item = $(this)[0].item;

                            if (listMap[id].F_ItemUrl) {
                                top.keren.frameTab.open({ F_ModuleId: 'dtlist' + item.f_id, F_FullName: item.f_title, F_UrlAddress: listMap[id].F_ItemUrl + item.f_id });
                            }
                            else {
                                top['dtlist' + item.f_id] = item;
                                top.keren.frameTab.open({ F_ModuleId: 'dtlist' + item.f_id, F_FullName: item.f_title, F_UrlAddress: '/Utility/ListContentIndex?id=' + item.f_id });
                            }
                            return false;
                        });
                    }
            ...
            $list.find('.kr-desktop-list .menu').on('click', function () {
                var $p = $(this).parents('.kr-desktop-list');
                var id = $p.attr('data-Id');
                top.keren.frameTab.open({ F_ModuleId: id, F_FullName: listMap[id].F_Name, F_UrlAddress: listMap[id].F_Url });
                return false;
            });
        }
    }
    ...
    function chart(data) {
        if (data.length > 0) {
            ...
            $('.kr-desktop-chat-panel-title .title-item').on('click', function (e) {
                var $this = $(this);
    ...
    // 获取配置数据
    top.keren.clientdata.getAsync('desktop', {
