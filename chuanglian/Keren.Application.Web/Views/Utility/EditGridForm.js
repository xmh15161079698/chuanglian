...
var bootstrap = function ($, keren) {
    ...
    function setRowOp() {
        ...
        $('#parentId').krselectRefresh({
            data: res
        });

        //$('#colop').krSetFormData(currentRow);
        $('#name').val(currentRow.name);
        $('#parentId').krselectSet(parentId);
        $('#field').krselectSet(currentRow.field);
        $('#align').krselectSet(currentRow.align);
        $('#width').val(currentRow.width);
        $('#type').krselectSet(currentRow.type);
        ...
        switch (currentRow.type) {
            ...
            case 'checkbox':
            case 'select':
                $('#dataSource').krselectSet(currentRow.dataSource);
                $('#dfvalue').krselectSet(currentRow.dfvalue);
                if (currentRow.dataSource === '0') {
                    $('#dataItemId').krselectSet(currentRow.itemCode);
                }
                else {
                    $('#dataSourceId').krselectSet(currentRow.dataSourceId);
                    $('#showField').krselectSet(currentRow.showField);
                    $('#saveField').krselectSet(currentRow.saveField);
                }
                break;
            case 'layer':
                ...
                break;
            case 'datetime':
                $('#datetime').krselectSet(currentRow.datetime);
                break;
        }
    ...
    var page = {
        ...
        bind: function () {
                ...
                select: function (item) {
                    ...
                    switch (item.id) {
                        ...
                        case 'select':
                            ...
                            $('#dataSource').krselectSet(currentRow.dataSource);
                            if (currentRow.dataSource === '0') {
                                $('#dataItemId').krselectSet(currentRow.itemCode || currentRow.dataItemCode);
                            }
                            else {
                                $('#dataSourceId').krselectSet(currentRow.dataSourceId);
                                $('#showField').krselectSet(currentRow.showField);
            ...
            $('#dataItemId').krselect({
                ...
                select: function (item) {
                    if (item) {
                        ...
                        if (currentRow.type === 'radio' || currentRow.type === 'checkbox' || currentRow.type === 'select') {
                            currentRow.dfvalue = '';
                            $('#dfvalue').krselectRefresh({ url: '', data: [] });
                        }
                    }
                }
            });
                    ...
                    if (item) {
                        ...
                        if (currentRow.type === 'radio' || currentRow.type === 'checkbox' || currentRow.type === 'select') {
                            ...
                            keren.httpAsync('GET', top.$.rootUrl + '/KR_SystemModule/DataSource/GetDataColName', { code: item.F_Code }, function (data) {
                                ...
                                $('#showField').krselectRefresh({
                                    data: fieldData,
                                    placeholder: false
                                });
                                $('#showField').krselectSet(currentRow.showField || fieldData[0].value);
                                $('#saveField').krselectRefresh({
                                    data: fieldData,
                                    placeholder: false
                                });
                                $('#saveField').krselectSet(currentRow.saveField || fieldData[0].value);
                            });
                        }
            ...
            if (!gridSetting.fieldMap[gridSetting.dbId + gridSetting.tableName]) {
                keren.httpAsync('GET', top.$.rootUrl + '/KR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: gridSetting.dbId, tableName: gridSetting.tableName }, function (data) {
