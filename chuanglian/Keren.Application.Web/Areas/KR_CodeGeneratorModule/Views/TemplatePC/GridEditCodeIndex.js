...
var bootstrap = function ($, keren) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        $('#step-2').krCustmerFormDesigner('updatedb', { dbId: databaseLinkId, dbTable: [{ name: dbTable }] });

                        if (mapField[databaseLinkId + dbTable]) {
                            $('#queryDatetime').krselectRefresh({ data: mapField[databaseLinkId + dbTable] });
                        }
                        else {
                            if (!mapField[databaseLinkId + dbTable]) {
                                keren.httpAsync('GET', top.$.rootUrl + '/KR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: dbTable }, function (data) {
                        ...
                        keren.httpAsyncPost(top.$.rootUrl + '/KR_CodeGeneratorModule/TemplatePC/LookGridEditCode', postData, function (res) {
                            if (res.code == 200) {
            ...
            // 表单设置
            $('#step-2').krCustmerFormDesigner('init', { components: ['text', 'radio', 'checkbox', 'select', 'datetime'] });
            ...
            $("#btn_save").on('click', function () {
                keren.layerForm({
                    id: 'codeform',
                    title: '保存模板',
                    url: top.$.rootUrl + '/KR_CodeGeneratorModule/CodeSchema/Form?F_Type=4',// 1.移动模板2.自定义模板3.工作流系统表单模板4.Excel风格模板5.报表模板
                    width: 600,
                    height: 400,
                    callBack: function (id) {
        ...
        save: function () {
            ...
            $.krSaveForm(top.$.rootUrl + '/KR_CodeGeneratorModule/TemplatePC/CreateGridEditCode', postData, function (res) { }, true);
        }
    };
