...
var bootstrap = function ($, keren) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            // 目标
            $('#F_Target').krselect().on('change', function () {
                // 目标改变
        ...
        initGrid: function () {
            $('#btns_girdtable').jfGrid({
                ...
                onAddRow: function (row, rows) {
                    row.F_ModuleButtonId = keren.newGuid();
                    row.F_ParentId = '';
                }
            });
            $('#view_girdtable').jfGrid({
                ...
                isEdit: true,
                onAddRow: function (row, rows) {
                    row.F_ModuleColumnId = keren.newGuid();
                    row.F_ParentId = '0';
                }
            });
            $('#form_girdtable').jfGrid({
                ...
                isEdit: true,
                onAddRow: function (row, rows) {
                    row.F_ModuleFormId = keren.newGuid();
                }
            });
        },
        /*初始化数据*/
        initData: function () {
            if (!!keyValue) {
                $.krSetForm(top.$.rootUrl + '/KR_SystemModule/Module/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('#step-1').krSetFormData(data.moduleEntity);
                ...
                $('#F_ParentId').krselectSet(moduleId);
            }
        },
        /*保存数据*/
        save: function () {
            ...
            $.krSaveForm(top.$.rootUrl + '/KR_SystemModule/Module/SaveForm', postData, function (res) {
                // 保存成功后才回调
                keren.frameTab.currentIframe().refreshGirdData(formdata);
            });
        }
    };
