...
var bootstrap = function ($, keren) {
    ...
    var page = {
        ...
        bind: function () {
            /*复制表*/
            $('#kr_copytable').on('click', function () {
                keren.layerForm({
                    id: 'copyform',
                    title: '复制表',
                    url: top.$.rootUrl + '/KR_SystemModule/DatabaseTable/CopyTableForm',
                    width: 800,
                    height: 600,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
                            $('#gridtable').jfGridSet('refreshdata', []);
                            keren.httpAsyncGet(top.$.rootUrl + '/KR_SystemModule/DatabaseTable/GetFieldList?databaseLinkId=' + data.databaseLinkId + '&tableName=' + data.tableName, function (res) {
            ...
            $('#kr_copyrow').on('click', function () {
                ...
                if (selectedRow.length > 0) {
                    ...
                    top.keren.alert.warning('请选择需要复制的行!');
                }
            });

            /*常用字段*/
            $('#kr_commonfield').on('click', function () {
                keren.layerForm({
                    id: 'commonfieldform',
                    title: '常用字段选择',
                    url: top.$.rootUrl + '/KR_SystemModule/DbField/SelectForm',
                    width: 600,
                    height: 500,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            /*导入草稿*/
            $('#kr_draft').on('click', function () {
                keren.layerForm({
                    id: 'draftform',
                    title: '导入草稿',
                    url: top.$.rootUrl + '/KR_SystemModule/DatabaseTable/DraftForm',
                    width: 800,
                    height: 600,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
                            if (data) {
                                draftId = data.F_Id;
                                $('#form').krSetFormData({ tableName: data.F_Name, remark: data.F_Remark});
            ...
            /*保存草稿*/
            $('#kr_savedraft').on('click', function () {
                if (!$('#form').krValidform()) {
                    return false;
                }
                ...
                $.krSaveForm(top.$.rootUrl + '/KR_SystemModule/DatabaseTable/SaveDraft?keyValue=' + draftId, postData, function (res) {
                    // 保存成功后才回调
                    if (res.code == 200) {
                        draftId = res.data;
                    }
            ...
            $('#kr_release').on('click', function () {
                ...
                $.krSaveForm(top.$.rootUrl + '/KR_SystemModule/DatabaseTable/SaveTable', postData, function (res) {
                    // 新建成功后刷新页面
                    if (res.code == 200) {
                        draftId = '';
                        $('#form').krSetFormData({ tableName: '', remark: '' });
                        $('#gridtable').jfGridSet('refreshdata', [{}]);
                    }
                },true);
            });
        ...
        checkGrid: function (data) {
            ...
            $.each(data, function (_index, _item) {
                ...
                if (_item.f_column === undefined || _item.f_column === '') {
                    keren.alert.error('【第' + _index + '列】列名为空');
                    flag = false;
                    return false;
                }
                    ...
                    keren.alert.error('【第' + _index + '列】数据类型为空');
                    flag = false;
                    return false;
                }
                else if (_item.f_datatype == 'varchar' && (_item.f_length == 0 || _item.f_length == undefined)) {
                    keren.alert.error('【第' + _index + '列】字串长度设置错误');
                    flag = false;
                    return false;
                }
                else if (map[_item.f_column] != undefined ){
                    keren.alert.error('【第' + _index + '列与第' + map[_item.f_column] + '列】列名重复');
                    flag = false;
                    return false;
                }
