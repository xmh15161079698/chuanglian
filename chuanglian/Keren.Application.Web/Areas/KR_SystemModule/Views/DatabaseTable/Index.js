    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#kr_add').on('click', function () {
                if (databaseLinkId == '') {
                    keren.alert.warning('请选择数据库!');
                } else {
                    keren.layerForm({
                        id: 'form',
                        title: '新建表',
                        url: top.$.rootUrl + '/KR_SystemModule/DatabaseTable/EditTableForm?databaseLinkId=' + databaseLinkId,
                        width: 800,
                        height: 600,
                        btn: null,
            ...
            /*打开表数据*/
            $('#kr_tabledata').on('click', function () {
                var tableName = $('#gridtable').jfGridValue('name');
                if (keren.checkrow(tableName)) {
                    keren.layerForm({
                        id: 'TableIndex',
                        title: '打开表数据【' + tableName + "】",
                        url: top.$.rootUrl + '/KR_SystemModule/DatabaseTable/TableIndex?tableName=' + tableName + '&databaseLinkId=' + databaseLinkId,
                        width: 1000,
                        height: 800,
                        maxmin: true,
                        btn: null
