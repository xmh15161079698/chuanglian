...
var bootstrap = function ($, keren) {
    ...
    var refreshData = function (label, data, rowid) {// 刷新条件
        if (rowid != "") {
            ...
            var $item = $('<div class="kr-query-item add-more-list" id="kr_query_item_' + rowid + '"><div class="list-text"></div><div class="edit-del"><a class="btn-edit">编辑</a>|<a class="btn-delete">删除</a></div></div>');
            $item.find('.btn-edit')[0].rowid = rowid;
            $item.find('.btn-delete')[0].rowid = rowid;
            $('#querylist').append($item);
        }
        $('#kr_query_item_' + rowid).find('.list-text').html('<i class="list-num">' + (rowid + 1) + '</i>' + label);
    };
    var loadData = function () {
        ...
        for (var i = 0, l = queryDataList.length; i < l; i++) {
            ...
            var $item = $('<div class="kr-query-item add-more-list" id="kr_query_item_' + i + '"><div class="list-text"></div><div class="edit-del"><a class="btn-edit">编辑</a>|<a class="btn-delete">删除</a></div></div>');
            $item.find('.btn-edit')[0].rowid = i;
            $item.find('.btn-delete')[0].rowid = i;
            $('#querylist').append($item);
            $('#kr_query_item_' + i).find('.list-text').html('<i class="list-num">' + (i + 1) + '</i>' + lablevalue);
        }
    }
    var page = {
        init: function () {
        ...
        bind: function () {
                ...
                if (data.direction == "next") {
                    ...
                    if (BaseData.F_IsRestart == "1") {
                        if (BaseData.F_MinuteValue == "" || BaseData.F_RestratValue == "") {
                            keren.alert.error('请检查重启时间或重启次数是否输入');
                            return false;
                        }
                    }
            ...
            $(".btn-select-express").on("click", function () {
                ...
                keren.layerForm({
                    ...
                    title: '查看预置表达式',
                    url: top.$.rootUrl + '/KR_TaskScheduling/TS_TaskDesign/SelectExpressForm',
                    width: 600,
                    height: 540,
            ...
            $('#F_CronExpression').krselect().on('change', function () {
                var corn = $('#F_CronExpression').krselectGet();
            ...
            $('#querylist').on('click', function (e) {
                ...
                if ($et.hasClass('btn-edit')) {
                    ...
                    keren.layerForm({
                        id: 'AddDetailedForm',
                        title: '编辑明细频率',
                        url: top.$.rootUrl + '/KR_TaskScheduling/TS_TaskDesign/AddDetailedForm?rowid=' + _rowid,
                        width: 500,
                        height: 300,
                        callBack: function (id) {
            ...
            $('#kr_query_add').on('click', function () {
                keren.layerForm({
                    id: 'AddDetailedForm',
                    title: '添加明细频率',
                    url: top.$.rootUrl + '/KR_TaskScheduling/TS_TaskDesign/AddDetailedForm',
                    width: 400,
                    height: 390,
                    callBack: function (id) {
                        return top[id].acceptClick(refreshData);
        ...
        initData: function () {
            if (!!selectedRow) {
                $('#step-1').krSetFormData(selectedRow);
                BaseData = JSON.parse(selectedRow.F_SchedulingPlan);
                $('#form').krSetFormData(BaseData);
                ///开始时间处理
                ...
                $("#step-2").krSetFormData(selectedRow);
                ///任务类型处理
                $("#db").hide();
                $("#sql").hide();
                $("#stored").hide();
    ...
    acceptClick = function (callBack) {
        ...
        var base = $('#step-1').krGetFormData();
        var excu = $("#step-2").krGetFormData();
        var postData = {
        ...
        $.krSaveForm(top.$.rootUrl + '/KR_TaskScheduling/TS_TaskDesign/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            keren.frameTab.currentIframe().refreshGirdData();//index页列表刷新
