...
var bootstrap = function ($, keren) {
    ...
    var page = {
        ...
        bind: function () {
                ...
                if (data.direction == "next") {
                    ...
                    switch (baseData.executeType) {
                        ...
                        case '2':// 简单重复执行
                            if (baseData.simpleValue == "") {
                                keren.alert.error('请填写间隔时间');
                                return false;
                            }
                            else if (!(/^[-+]?\d+$/).test(baseData.simpleValue)) {
                                keren.alert.error('间隔时间必须为数字');
                                return false;
                            }
                            break;
                        case '3':// 明细频率设置
                            if (frequencyList.length <= 0) {
                                keren.alert.error('请添加明细频率');
                                return false;
                            } else {
                    ...
                    if (baseData.isRestart == "1") {
                        if (baseData.restartMinute == "") {
                            keren.alert.error('请填写重启间隔时间');
                            return false;
                        }

                        if (baseData.restartNum == "") {
                            keren.alert.error('请填写重启次数');
                            return false;
                        }
                    }
            ...
            // 明细频率设置
            $('#kr_frequency_add').on('click', function () {
                keren.layerForm({
                    id: 'AddDetailedForm',
                    title: '添加明细频率',
                    url: top.$.rootUrl + '/KR_TaskScheduling/TSScheme/AddDetailedForm',
                    width: 600,
                    height: 400,
            ...
            $('#frequencyList').on('click', '.edit-del a', function () {
                ...
                if ($this.hasClass('btn-edit')) {
                    keren.layerForm({
                        id: 'AddDetailedForm',
                        title: '编辑明细频率',
                        url: top.$.rootUrl + '/KR_TaskScheduling/TSScheme/AddDetailedForm?rowid=' + _rowid,
                        width: 600,
                        height: 400,
                        callBack: function (id) {
                            return top[id].acceptClick(loadFrequencyData);
            ...
            $(".btn-select-express").on("click", function () {
                keren.layerForm({
                    ...
                    title: '添加预置表达式',
                    url: top.$.rootUrl + '/KR_TaskScheduling/TSScheme/SelectExpressForm',
                    width: 600,
                    height: 500,
                    btn: null
                });
        ...
        initData: function () {
            if (keyValue) {
                $.krSetForm(top.$.rootUrl + '/KR_TaskScheduling/TSScheme/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('#wizard-steps').krSetFormData(data.schemeInfoEntity);
                    var scheme = JSON.parse(data.schemeEntity.F_Scheme);
                    $('#wizard-steps').krSetFormData(scheme);
    ...
    acceptClick = function (callBack) {
        ...
        $.krSaveForm(top.$.rootUrl + '/KR_TaskScheduling/TSScheme/SaveForm', postData, function (res) {
            // 保存成功后才回调
