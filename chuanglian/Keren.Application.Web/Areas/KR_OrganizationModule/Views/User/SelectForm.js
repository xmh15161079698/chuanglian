...
var bootstrap = function ($, keren) {
    ...
    function renderSelectedUserlist() {
        ...
        if (!!selectValue)
        {
            keren.httpAsync('GET', top.$.rootUrl + '/KR_OrganizationModule/User/GetEntityListByUserIds', { keyValue: selectValue }, function (data) {
                if (!!data) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#department_tree').krtree({
                nodeClick: function (item) {
                    ...
                    if (!!userlist[item.id]) {
                        ...
                        keren.httpAsync('GET', top.$.rootUrl + '/KR_OrganizationModule/User/GetList', { companyId: companyId, departmentId: departmentId }, function (data) {
            ...
            // 公司
            $('#company_select').krCompanySelect().bind('change', function () {
            ...
            $("#txt_keyword").keydown(function (event) {
                if (event.keyCode == 13) {
                    ...
                    if (keyword != "") {
                        keren.httpAsync('GET', top.$.rootUrl + '/KR_OrganizationModule/User/GetList', { companyId: companyId, keyword: keyword }, function (data) {
                            renderUserlist(data || []);
                        });
                    }
                    else {
            ...
            $('#selected_user_list_warp').krscroll();
        },
        initData: function () {
            ...
            if (!!companyId) {
                ...
                $('#department_tree').krtreeSet('setValue', departmentId);
            }
            else {
                var loginInfo = keren.clientdata.get(['userinfo']);
                if (!!loginInfo && !!loginInfo.F_CompanyId && !!loginInfo.F_DepartmentId) {
                    $('#company_select').krselectSet(loginInfo.F_CompanyId);
                    $('#department_tree').krtreeSet('setValue', loginInfo.F_DepartmentId);
                }
            }
            // 初始化选中的人员
