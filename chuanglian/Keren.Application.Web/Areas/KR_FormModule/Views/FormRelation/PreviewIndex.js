...
var bootstrap = function ($, keren) {
    ...
    var page = {
        init: function () {
            // 获取自定义表单设置内容
            keren.httpAsync('GET', top.$.rootUrl + '/KR_FormModule/FormRelation/GetCustmerFormData', { keyValue: id }, function (data) {
                ...
                if (settingJson.query.isDate == '1' && !!settingJson.query.DateField) {// 时间搜索框
                    $('#datesearch').krdate({
                        ...
                        selectfn: function (begin, end) {
                            queryJson.krbegin = begin;
                            queryJson.krend = end;
                            queryJson.krdateField = settingJson.query.DateField;
                ...
                if (!!settingJson.query.fields && settingJson.query.fields.length > 0) {
                    ...
                    $('#multiple_condition_query_item').html('<div id="multiple_condition_query"><div class="kr-query-formcontent"></div></div>');
                    $('#multiple_condition_query').krMultipleQuery(function (_queryJson) {
                        queryJson = _queryJson;
                        page.search();
                    }, height);
                    var $content = $('#multiple_condition_query .kr-query-content');
                    ...
                    $.each(queryFieldMap, function (id, item) {
                        if (!!item.compont) {
                            var $row = $('<div class="col-xs-' + (12 / parseInt(item.portion)) + ' kr-form-item" ></div>');
                            var $title = $(' <div class="kr-form-item-title">' + item.fieldName + '</div>');
                            $row.append($title);
                            $content.append($row);
                            $.krFormComponents[item.compont.type].renderQuery(item.compont, $row)[0].compont = item.compont;
                        }
                    });
                }
                // 列表设置
            ...
            $('#kr_add').on('click', function () {
                if (settingJson.layer.opentype == '1') {// 窗口弹层页
                    keren.layerForm({
                        id: 'Form',
                        title: '新增',
                        url: top.$.rootUrl + '/KR_FormModule/Custmerform/LayerInstanceForm?id=' + relation.F_FormId,
                    ...
                    keren.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '新增', F_UrlAddress: '/KR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId });
                }
            });
            // 编辑
                ...
                if (keren.checkrow(keyValue)) {
                    if (settingJson.layer.opentype == '1') {// 窗口弹层页
                        keren.layerForm({
                            id: 'Form',
                            title: '编辑',
                            url: top.$.rootUrl + '/KR_FormModule/Custmerform/LayerInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue,
                            width: settingJson.layer.width,
                            height: settingJson.layer.height,
                            callBack: function (id) {
                                return top[id].acceptClick(refreshGirdData);
                        ...
                        keren.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '编辑', F_UrlAddress: '/KR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue });
                    }
                }
            });
            // 删除
            $('#kr_delete').on('click', function () {
                ...
                if (keren.checkrow(keyValue)) {
                    keren.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            keren.deleteForm(top.$.rootUrl + '/KR_FormModule/Custmerform/DeleteInstanceForm', { keyValue: keyValue, schemeInfoId: relation.F_FormId }, function () {
                                refreshGirdData();
                            });
                        }
                    });
        ...
        initGrid: function () {
                    ...
                    switch (item.compont.type) {
                        ...
                        case 'select':
                            if (item.compont.dataSource == "0") {
                                ...
                                point.formatterAsync = function (callback, value, row) {
                                    keren.clientdata.getAsync('sourceData', {
                                        key: value,
                                        keyId: vlist[2],
                                        code: vlist[0],
                                        callback: function (_data) {
            ...
            if (settingJson.col.isPage == "1") {
                girdurl = top.$.rootUrl + '/KR_FormModule/FormRelation/GetPreviewPageList?keyValue=' + id;
            }
            else {
                girdurl = top.$.rootUrl + '/KR_FormModule/FormRelation/GetPreviewList?keyValue=' + id;
            }
