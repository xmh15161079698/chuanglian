...
function stopLottery() {
    ...
    // 随机生成停止位置的索引
    if (lottery_size > 6) {
        var stop_index = Math.floor(Math.random() * (lottery_size - 6));
    } else {
    ...
    $.ajax({
        url: "/KR_CodeModule/WD_ChouJiangPage/SaveZhongJiangUsr",
        data: {
...
$(function () {
    ...
    var lottery_btn_fun = function () {
        ...
        $.ajax({
            url: "/KR_CodeModule/WD_ChouJiangPage/CanChouJiang",
            type: "get",
            dataType: "json",
            success: function (datas) {
                // alert(datas.data.length)
                ...
                if ($('#lottery-btn').text() == '可抽奖啦 @_@') {
                    if (award_log['award0' + cur_lottery] <= 1) {
                        ...
                        if (cur_lottery < 1) {
                            ...
                            if (cur_lottery == 1) {
                                message = '奖品已经抽完啦！！';
                            } else if (cur_lottery == 2) {
                                if (award_log['award0' + (cur_lottery - 1)] != 0) {
                                    message = '二等奖已经抽完，下次抽取一等奖';
                                } else {
                                    message = '奖品已经抽完啦！！';
                                }
                              
                            } else if (cur_lottery == 3) {
                                if (award_log['award0' + (cur_lottery - 1)] != 0) {
                                    message = '三等奖已经抽完，下次抽取二等奖';
                                } else if (award_log['award0' + (cur_lottery - 2)] != 0) {
                                    message = '三等奖已经抽完，下次抽取一等奖';
                                } else {
                                    message = '奖品已经抽完啦！！';
                                }
                            }
                            ...
                            //$('#lottery-btn').data('award', cur_lottery - 1);
