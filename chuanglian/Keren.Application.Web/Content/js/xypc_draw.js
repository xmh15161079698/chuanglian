...
function stopLottery() {
    ...
    var lottery_initial_total = local_handle.get('lottery_initial');
    lottery_initial_total = JSON.parse(lottery_initial_total);
    //alert(lottery_initial_total.length)
    if (lottery_initial_total.length < 6) {
        var zhongJiangUsr = [];
        for (var i = 0; i < lottery_initial_total.length; i++) {
            var zhongjiang2 = lottery_initial_total[i];
            var usr1 = {
                'openId': zhongjiang2.openId,
                'awardType': award
            }
            zhongJiangUsr.push(usr1)
        }
    } else {
        zhongJiangUsr = [];
        zhongJiangUsr.push({ 'openId': lottery_name_openId, 'awardType': award })
        lottery_initial_total = local_handle.delete(lottery_initial_total, lottery_name_en);
        for (var i = 0; i < 4; i++) {
            
            var rand = Math.floor(Math.random() * (lottery_initial_total.length));
            var zhongjiang2 = lottery_initial_total[rand];
            var usr1 = {
                'openId': zhongjiang2.openId,
                'awardType': award
            }
            zhongJiangUsr.push(usr1)
            lottery_initial_total = local_handle.delete(lottery_initial_total, zhongjiang2.nameen);
        } 
    }
    console.log('zhongjiangUsr=', zhongJiangUsr)
    var zhongjiangJson = {
        'zhongJiangUsr': zhongJiangUsr
    }
    zhongjiangJson = JSON.stringify(zhongjiangJson)
    $.ajax({
        url: "/KR_CodeModule/WD_XingYunChouJiangPage/SaveZhongJiangUsr",
        data: {
            'zhongjiangJsonPro': zhongjiangJson
        },
        type: "post",
