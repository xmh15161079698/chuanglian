...
var award_log = window.localStorage.getItem('award_now_count');
...
function stopLottery() {
    ...
    alert(lottery_size)
    // 随机生成停止位置的索引
    if (lottery_size !== 16 && lottery_size != 8 && lottery_size !== 12 && lottery_size != 6 && lottery_size !== 8 && lottery_size != 4 && lottery_size !== 4 && lottery_size != 2) {
    ...
    if (lottery_size !== 16 && lottery_size != 8 && lottery_size !== 12 && lottery_size != 6 && lottery_size !== 8 && lottery_size != 4 && lottery_size !== 4 && lottery_size != 2) {
        ...
        //var sure_index1 = stop_index+2;
        //var sure_index3 = stop_index+3;
        //var sure_index4 = stop_index + 5;
        //var sure_index5 = stop_index + 6;
    } else {
        // 将整个抽奖块移动到停止索引所在位置 top 值
        var stop_top = 0;
        //        $('#lottery-wrap').css('top', -stop_top);
        ...
        var sure_index = null;
        var sure_index1 = null;
        var sure_index3 = null;
        var sure_index4 = null;
        if (lottery_size == 16 || lottery_size == 8) {
            sure_index = stop_index;
            sure_index1 = stop_index - 2;
            sure_index3 = stop_index - 1;
            sure_index4 = stop_index + 1; 
        }
        if (lottery_size == 12 || lottery_size == 6) {
            sure_index = stop_index;
            sure_index3 = stop_index - 1;
            sure_index4 = stop_index + 1;
        }
        if (lottery_size == 8 || lottery_size == 4) {
            sure_index = stop_index;
            sure_index3 = stop_index - 1; 
        }
        if (lottery_size == 4 || lottery_size == 2) {
            sure_index = stop_index;
        }
    }
    ...
    var lastStep = function () {
        time02 = nextFrame(function () {
            ...
            if (-top <= left_distance) {
                ...
                if (sure_index1 != null) {
                    $('#lottery-wrap .lottery-list').eq(sure_index1).addClass('sure-active');
                }
                if (sure_index3 != null) {
                    $('#lottery-wrap .lottery-list').eq(sure_index3).addClass('sure-active');
                }
                if (sure_index4 != null) {
                    $('#lottery-wrap .lottery-list').eq(sure_index4).addClass('sure-active');
                }
                if (sure_index5 != null) {
                    $('#lottery-wrap .lottery-list').eq(sure_index5).addClass('sure-active');
                }
                var award_tpl = $('#awardcon-tpl').html();
                var award_dom = substitute(award_tpl, award_tmp);
                var award_dom1 = substitute(award_tpl, award_tmp1);
                var award_dom3 = substitute(award_tpl, award_tmp3);
                var award_dom4 = substitute(award_tpl, award_tmp4);
                var award_dom5 = substitute(award_tpl, award_tmp5);
                $('#award-0' + award).show();
                //if (award == 4) {
                //    $('#award-123').hide();
                //    $('#award-04').show();
                ...
                $('#award-0' + award + ' .win').append(award_dom1);
                $('#award-0' + award + ' .win').append(award_dom3);
                $('#award-0' + award + ' .win').append(award_dom4);
                $('#award-0' + award + ' .win').append(award_dom5);
    ...
    var lottery_name_zh1 = $('#lottery-wrap .lottery-list').eq(sure_index1).data('namezh');
    var lottery_name_zh3 = $('#lottery-wrap .lottery-list').eq(sure_index3).data('namezh');
    var lottery_name_zh4 = $('#lottery-wrap .lottery-list').eq(sure_index4).data('namezh');
    var lottery_name_zh5 = $('#lottery-wrap .lottery-list').eq(sure_index5).data('namezh');
    var lottery_name_en = $('#lottery-wrap .lottery-list').eq(sure_index).data('nameen');
    var lottery_name_en1 = $('#lottery-wrap .lottery-list').eq(sure_index1).data('nameen');
    var lottery_name_en3 = $('#lottery-wrap .lottery-list').eq(sure_index3).data('nameen');
    var lottery_name_en4 = $('#lottery-wrap .lottery-list').eq(sure_index4).data('nameen');
    var lottery_name_en5 = $('#lottery-wrap .lottery-list').eq(sure_index5).data('nameen');
    var lottery_touxiang = $('#lottery-wrap .lottery-list').eq(sure_index).data('touxiang');
    var lottery_touxiang1 = $('#lottery-wrap .lottery-list').eq(sure_index1).data('touxiang')
    var lottery_touxiang3 = $('#lottery-wrap .lottery-list').eq(sure_index3).data('touxiang')
    var lottery_touxiang4 = $('#lottery-wrap .lottery-list').eq(sure_index4).data('touxiang')
    var lottery_touxiang5 = $('#lottery-wrap .lottery-list').eq(sure_index5).data('touxiang')

    var lottery_name_openId = $('#lottery-wrap .lottery-list').eq(sure_index).data('openid');
    var lottery_name_openId1 = $('#lottery-wrap .lottery-list').eq(sure_index1).data('openid');
    var lottery_name_openId3 = $('#lottery-wrap .lottery-list').eq(sure_index3).data('openid');
    var lottery_name_openId4 = $('#lottery-wrap .lottery-list').eq(sure_index4).data('openid');
    var lottery_name_openId5 = $('#lottery-wrap .lottery-list').eq(sure_index5).data('openid');
    var data = {
        'lottery_name_zh1': lottery_name_zh1,
        'lottery_name_zh': lottery_name_zh,
        'lottery_name_zh3': lottery_name_zh3,
        'lottery_name_zh4': lottery_name_zh4,
        'lottery_name_zh5': lottery_name_zh5
    }
    data = JSON.stringify(data)
    console.log("data =",data);
    //$.ajax({
    //    url: "/KR_CodeModule/WD_XingYunChouJiangPage/SaveZhongJiangUsr",
    //    data: [
    //        {
    //        "openId": lottery_name_openId,
    //        "awardType": award
    //        },
    //        {
    //            "openId1": lottery_name_openId1,
    //            "awardType1": award
    //        },
    //        {
    //            "openId3": lottery_name_openId3,
    //            "awardType3": award
    //        },
    //        {
    //            "openId4": lottery_name_openId4,
    //            "awardType4": award
    //        },
    //        {
    //            "openId5": lottery_name_openId5,
    //            "awardType5": award
    //        }
    //    ],
    //    type: "post",
    ...
    //                var lottery_initial_two = [];
    //                // alert(datas.data.length)
    //                for (var i = 0; i < datas.data.length; i++) {

    //                    lottery_initial_two[i] = { "nameen": "avatar" + (i + 1), "namezh": datas.data[i].F_NickName, "touxiang": datas.data[i].F_AvatarUrl, "openId": datas.data[i].F_OpenId }
    //                }

    //                var data_str = JSON.stringify(lottery_initial_two);
        ...
        award_tmp =
            {
                ...
                'openId': lottery_name_openId,
                'touxiang': lottery_touxiang

            };
        award_tmp1 = {
            'nameen1': lottery_name_en1,
            'namezh1': lottery_name_zh1,
            'openId1': lottery_name_openId1,
            'touxiang1': lottery_touxiang1
        };
        award_tmp3 = {
            'nameen3': lottery_name_en3,
            'namezh3': lottery_name_zh3,
            'openId3': lottery_name_openId3,
            'touxiang3': lottery_touxiang3
        };
        award_tmp4 = {
            'nameen4': lottery_name_en4,
            'namezh4': lottery_name_zh4,
            'openId4': lottery_name_openId4,
            'touxiang4': lottery_touxiang4
        };
        award_tmp5 = {
            'nameen5': lottery_name_en5,
            'namezh5': lottery_name_zh5,
            'openId5': lottery_name_openId5,
            'touxiang5': lottery_touxiang5
        };
        
        award_datas.push(award_tmp);
        award_datas.push(award_tmp1);
        award_datas.push(award_tmp3);
        award_datas.push(award_tmp4);
        award_datas.push(award_tmp5);
    ...
    local_handle.set('award_now_count', JSON.stringify(award_log));
    ...
    //setTimeout(function () {
    //    //$("#lottery-result").css("opacity", "0.5");
    //    $("#canvas").css("opacity", "0.7");
    //    $(".modal-backdrop.in").css("opacity", "0.5");
    //    can_stop = true;
    //    clearTimeout(arguments.callee);
    //    //中奖信息
    //    drawAward(lottery_name_zh, lottery_touxiang);
...
// canvas 绘制中奖结果
function drawAward(name_zh, touxiang, pic_format) {
    ...
    back_img.src = '/Content/img/award_' + 0 + '.' + pic_format;
    back_img.onload = function () {
        ...
        context.font = "bold 25px fantasy";
        //          if (name_zh.length <= 2) {
        ...
        context.fillText(name_str, 160, 350);
        //alert(touxiang)
        var img = new Image();
        img.src = "" + touxiang + "";
        context.drawImage(img, 100, 320, 50, 50);
...
function showMessage(message) {
    var messageJQ = $("<div style=' padding: 10px 20px;border - radius: 5px; position: fixed;top: 25%;left: 40%;color: #000;font-family:微软雅黑;font-size:20px;z-index: 99999;transform: translate(-50 %, 0);background-color: #f8f8f8; border: 1px solid #e7e7e7;'>" + message + "</div>");

    // 先将原始隐藏，然后添加到页面，最后以400毫秒的速度下拉显示出来
    messageJQ.hide().appendTo("#promptInfo").slideDown(400);
    // 4秒之后自动删除生成的元素
...
requestFullScreen();
$(function () {
    var fullhtml = '<div class="kr_frame_fullscreen" style="float:left;margin-right:20px"><a href="javascript:void(0);" id="kr_fullscreen_btn" title="全屏"><i class="fa fa-arrows-alt"></i></a></div>';
    $('#fullScreenAndRefresh').append(fullhtml);
    //alert(111)
    ...
    var refrehtml = '<div class="kr_frame_refresh" style="float:right"><a href="javascript:void(0);" id="kr_refresh_btn" title="刷新"><i class="fa fa-refresh"></i></a></div>';
    $('#fullScreenAndRefresh').append(refrehtml);
    $('#kr_refresh_btn').on('click', function () {
        location.reload();
    });
    ...
    // } else {
    //     $('.award').eq(2).addClass('award-active');
    //     $('#lottery-btn').data('award', 3);
    // }

    
    //alert($('#lottery-btn').data('award'))
    ...
    var lottery_btn_fun = function () {
        ...
        if (select_award) {
            ...
            $('#lottery-btn').data('award', 99);
        }
        $.ajax({
            url: "/KR_CodeModule/WD_XingYunChouJiangPage/CanChouJiang",
            type: "get",
            dataType: "json",
            success: function (datas) {
                // alert(datas.data.length)
                ...
                canchoujiang_datas[0] = { "code": datas.code, "info": datas.info }

                localStorage.removeItem('canchoujiang_datas');
                if (!localStorage.getItem('canchoujiang_datas')) {

                    var data_str = JSON.stringify(canchoujiang_datas);

                    localStorage.setItem('canchoujiang_datas', data_str);
                }
                var canchoujiangdatas = local_handle.get("canchoujiang_datas");
                //alert(111)
                var canchoujiangjson = JSON.parse(canchoujiangdatas)
                if (canchoujiangjson[0].code != 200) {
                    
                    showMessage('奖品已经全部抽完 ^_^！');
                    return;
                }
               // var lottery_size = $('#lottery-wrap .lottery-list').size();
                var cur_lottery = $('#lottery-btn').data('award');
                //alert('cur_lottery='+cur_lottery)

                if (!cur_lottery) {
                    showMessage('请先设置好奖项再抽奖哦 ^ _ ^');

                    return;
                }
                var award_log = window.localStorage.getItem('award_now_count');
                award_log = JSON.parse(award_log)
                if ($('#lottery-btn').text() == '可抽奖啦 @_@') {
                    if (award_log['award0' + cur_lottery] <= 1) {
    ...
    //var lottery_storage = window.localStorage.getItem('lottery_initial');
    //var lottery_data = JSON.parse(lottery_storage);
    //$('#lottery-wrap').html(_.template($('#lotterycon-tpl').html(), lottery_data));
    //$('#lottery-wrap').html($('#lottery-wrap').html() + $('#lottery-wrap').html());
