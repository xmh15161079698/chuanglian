    ...
    public class CompanyService : RepositoryFactory
    {
        ...
        public IEnumerable<CompanyEntity> GetList()
        {
            try
            {
                ...
                strSql.Append(" FROM KR_Base_Company t WHERE t.F_EnabledMark = 1 AND t.F_DeleteMark = 0  ORDER BY t.F_ParentId,t.F_FullName ");
        ...
        public IEnumerable<CompanyEntity> GetWeChatList(string keyword)
        {
            try
            {
                var sql = @"SELECT t.F_CompanyId,t.F_ParentId,t.F_EnCode,t.F_FullName,t.F_Fax FROM KR_Base_Company t WHERE t.F_EnabledMark = 1 AND t.F_DeleteMark = 0  AND t.F_EnCode IS NOT NULL 
                            UNION ALL
                            SELECT m.F_DepartmentId as F_CompanyId,m.F_CompanyId as F_ParentId,m.F_EnCode,m.F_FullName,m.F_Fax FROM KR_Base_Department m WHERE m.F_EnabledMark = 1 AND m.F_DeleteMark = 0 AND m.F_EnCode IS NOT NULL
