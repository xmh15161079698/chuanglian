    ...
    public class DepartmentService : RepositoryFactory
    {
        ...
        public IEnumerable<DepartmentEntity> GetList(string companyId)
        {
            try
            {
                ...
                strSql.Append(" FROM KR_Base_Department t WHERE t.F_EnabledMark = 1 AND t.F_DeleteMark = 0 AND F_CompanyId = @companyId ORDER BY t.F_ParentId,t.F_FullName ");
        ...
        public IEnumerable<DepartmentEntity> GetAllList()
        {
            try
            {
                ...
                strSql.Append("SELECT ");
                strSql.Append(fieldSql);
                strSql.Append(" FROM KR_Base_Department t WHERE t.F_EnabledMark = 1 AND t.F_DeleteMark = 0 ");
